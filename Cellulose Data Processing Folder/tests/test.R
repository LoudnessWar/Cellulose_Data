...
find_files <- function(directories, file_pattern) {
  matching_files <- list()
  for (directory in directories) {
    files <- list.files(path = directory, pattern = file_pattern, full.names = TRUE)
    matching_files[[directory]] <- files
  }
...
if (length(args) == 0) {
  cat("To use: Thinger thinger.R directory1 directory2 ...\n")
  q(save = "no")
}

#directory <- "DSI_Data" old news
file_pattern <- '*.dat'

matching_files <- find_files(directories, file_pattern)

for (directory in names(matching_files)) {
  cat("Matching files found or whatever BAKA!!!!! xb", directory, ":\n")
